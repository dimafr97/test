...
export function initPreviewThree(container, size) {
  ...
  // CAD overlay group (для линий/точек превью)
...
export function clearPreviewCadOverlay(three) {
  ...
  // удаляем только CAD-объекты, не трогая модель и свет
...
// Толстые линии + точки для превью (везде работает: это Mesh-геометрия)
export function setPreviewCadOverlay(three, cadSpec, opts = {}) {
  ...
  // Базовый масштаб под текущую модель (чтобы CAD не был гигантским/микро)
  let baseR = 1;
  if (three.currentModel) {
    const box = new THREE.Box3().setFromObject(three.currentModel);
    const sphere = box.getBoundingSphere(new THREE.Sphere());
    baseR = Math.max(0.001, sphere.radius || 1);
  }

  const pointR = Math.max(0.002, baseR * 0.018); // точки
  const lineR  = Math.max(0.0015, baseR * 0.006); // линии
  ...
  // ---- точки ----
  const pointGeo = new THREE.SphereGeometry(pointR, 16, 12);

  const pointMap = new Map(); // id -> position(Vector3)
  for (const p of cadSpec.points) {
    const m = new THREE.Mesh(pointGeo, mat);
    m.position.set(p.x, p.y, p.z);
    m.renderOrder = 1000;
    three.cadGroup.add(m);
    pointMap.set(String(p.id), m.position);
  }
  ...
  for (const seg of lines) {
    const aId = String(seg[0]);
    const bId = String(seg[1]);
    const pa = pointMap.get(aId);
    const pb = pointMap.get(bId);
    if (!pa || !pb) continue;
    ...
    const cylGeo = new THREE.CylinderGeometry(lineR, lineR, len, 12, 1, true);
    const cyl = new THREE.Mesh(cylGeo, mat);

    tmpMid.addVectors(pa, pb).multiplyScalar(0.5);
    cyl.position.copy(tmpMid);

    tmpQuat.setFromUnitVectors(up, tmpDir.normalize());
    cyl.quaternion.copy(tmpQuat);

    cyl.renderOrder = 999;
    three.cadGroup.add(cyl);
  }
}
